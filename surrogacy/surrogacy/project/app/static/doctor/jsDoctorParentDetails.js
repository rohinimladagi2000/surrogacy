...
$(document).ready(function () {
  ...
  $(document).on("click", "#btn_update", function () {
    ...
    if ($("#selSurrogates").val().trim().length < 1) {
      alert("Please Select Surrogates");
      $("#selSurrogates").focus();
      return false;
    }
    ...
    formData.append("selSurrogates", $("#selSurrogates").val());
    formData.append("csrfmiddlewaretoken", $('input[name=csrfmiddlewaretoken]').val());

    $.ajax({

        url: "/assign_surrogates/",
        type: "POST",
        data: formData,
        processData: false,
        contentType: false,
        success: function (response) {
          ...
          alert("Surrogate Assigned succesfully");
          location.reload();
          table.ajax.reload();
        },
        error: function (request, error) {
...
getAdminData();
  ...
  function getDoctorDetails(){
    ...
    formData.append("csrfmiddlewaretoken", $('input[name=csrfmiddlewaretoken]').val());
      ...
      url: "/get_data/hospital_surrogates/",
      type: "POST",
      data: formData,
      processData: false,
      contentType: false,
      ...
      for(var i = 0; i < response.length; i++) {
          var j = i + 1;
          $("#selSurrogates").append("<option>"+response[i].sr_name+"</option>")

...
$("#btn_add").click(function (e) {
  ...
  if ($("#uploadAadhar").val().trim().length < 1) {
  ...
  var lclFile = document.getElementById("uploadAadhar");
  lclAadhar = lclFile.files[0]; 
  ...
  formData.append("txtAadhar",lclAadhar);
  ...
  $.ajax({
    ...
    success: function (result) {
        ...
        alert("Surrogate details Added Successfully");
        location.reload();
        table.ajax.reload();
...
$(document).ready(function () {
  ...
  $(document).on("click", "#btn_update", function () {
    ...
    var formData = new FormData()
    formData.append("selDoctor1", $("#selDoctor1").val());
    ...
    $.ajax({
      ...
      contentType: false,
      success: function (result) {
        alert("Surrogate Details Updated Succesfully");
        location.reload();
        table.ajax.reload();
  ...
  $(document).on("click", "#btn_delete", function () {
    ...
    $.ajax({
      ...
      contentType: false,
      success: function () {
        alert("Surrogate Details deleted succesfully");
        location.reload();
        table.ajax.reload();
  ...
  $.ajax({
      ...
      success: function (response) {
        ...
        for(var i = 0; i < response.length; i++) {
          var j = i + 1;
          $("#tableData").append('<tr><td>'+j+'</td><td style="display: none;">'+response[i].sr_id+'</td><td>'+response[i].sr_name+'</td><td>'+response[i].sr_email+'</td><td>'+response[i].sr_mobile+'</td><td><a href="'+response[i].sr_aadhar+'">Download</a></td><td>'+response[i].sr_doctor+'</td><td><div class="d-flex" style="justify-content: space-evenly;"><a href="javascript:void(0);" id="edit_row" title="View/Edit" data-bs-toggle="modal" data-bs-target="#edit_modal"  class="text-primary" onClick="getRowsUpdate();">Edit</a><a href="javascript:void(0);" title="Delete" data-bs-toggle="modal" data-bs-target="#delete_modal" class="text-danger" id="delete_row" onClick="getRowsDelete();">Delete</a></div></td></tr>');
        }
      },
      error: function (request, error) {
        console.error(error);
  ...
  function getDoctorDetails(){
    ...
    formData.append("csrfmiddlewaretoken", $('input[name=csrfmiddlewaretoken]').val());
      ...
      for(var i = 0; i < response.length; i++) {
          ...
          $("#selDoctor1").append("<option>"+response[i].dc_name+"</option>");
